/**
 * Capture App Store screenshots at iPhone 15 Pro Max dimensions (1290×2796)
 * Compatible with the "6.5-inch display" requirement (1242×2688, 1284×2778, or 1290×2796).
 *
 * Usage:  node capture-appstore-screenshots.js
 * Output: pwa-packages/ios/screenshots/
 */
...
const OUTPUT_DIR = path.resolve(__dirname, '..', 'pwa-packages', 'ios', 'screenshots');

// iPhone 15 Pro Max logical resolution at 3× scale → 1290×2796 actual pixels
const VIEWPORT = { width: 430, height: 932, deviceScaleFactor: 3 };

async function capture() {
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
  console.log(`Capturing App Store screenshots from ${SITE_URL}`);
  ...
  // Type a destination to trigger route mode
  ...
  if (searchInput) {
    ...
    await wait(3000);
    // Try to pick the first autocomplete suggestion
    await page.evaluate(() => {
  ...
  console.log('\nThese are ready for the App Store "6.5-inch Display" slot.');
}
